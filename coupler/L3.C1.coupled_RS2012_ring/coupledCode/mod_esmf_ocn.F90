...
!
      ...
      real(ESMF_KIND_R8), pointer :: ptr_evap(:,:), ptr_rainc(:,:)
      real(ESMF_KIND_R8), pointer :: ptr_rainsh(:,:), ptr_rainnc(:,:)
      real(ESMF_KIND_R8), pointer :: ptr_sst_ini(:,:)
!
      type(ESMF_VM) :: vm
      type(ESMF_Clock) :: clock
      type(ESMF_Grid) :: gridIn
      ...
      type(ESMF_Field) :: field_evap, field_rainc
      type(ESMF_Field) :: field_rainsh, field_rainnc
      type(ESMF_Field) :: field_sst_ini
      ...
      REAL*8, DIMENSION(:,:,:,:), ALLOCATABLE :: sst_ini_ESMF
      integer :: myThid = 1
!
      rc = ESMF_SUCCESS
!
...
!
      ...
      ALLOCATE(sst_ini_ESMF(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy))
...
!
      ...
      call ESMF_StateGet(importState, "SFCEVP", field_evap, rc=rc)
      call ESMF_StateGet(importState, "RAINC", field_rainc, rc=rc)
      call ESMF_StateGet(importState, "RAINSH", field_rainsh, rc=rc)
      call ESMF_StateGet(importState, "RAINNC", field_rainnc, rc=rc)
...
!       ! Get pointer /from field
        ...
        call ESMF_FieldGet(field_rainc, localDE=j, farrayPtr=ptr_rainc, rc=rc)
        call ESMF_FieldGet(field_rainsh, localDE=j, farrayPtr=ptr_rainsh, rc=rc)
        call ESMF_FieldGet(field_rainnc, localDE=j, farrayPtr=ptr_rainnc, rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU,  &
            line=__LINE__, file=FILENAME)) return
...
!
        ...
        where (ieee_is_nan(ptr_rainc)) ptr_rainc = MISSING_R8
        where (ieee_is_nan(ptr_rainsh)) ptr_rainsh = MISSING_R8
        where (ieee_is_nan(ptr_rainnc)) ptr_rainnc = MISSING_R8
...
!
        do jj = 1, sNy
          do ii = 1, sNx
            ...
            !! total_evap_ESMF(ii,jj,1,1) = 0.0d0
            !! precip_ESMF(ii,jj,1,1) = 0.0d0
            !! total_precip_ESMF(ii,jj,1,1) = 0.0d0
            !! sst_ini_ESMF(ii,jj,1,1) = 0.0d0
            if ((iG > 0 .and. iG < imax) .and.                          &
                (jG > 0 .and. jG < jmax) .and.                          &
                 ptr_lwup(iG,jG) < TOL_R8 .and. ptr_lwdn(iG,jG) < TOL_R8 ) then
                lwdown_ESMF(ii,jj,1,1) = (ptr_lwup(iG,jG)-ptr_lwdn(iG,jG))*sfac+addo
                !! lwdown_ESMF(ii,jj,1,1) = (ptr_lwup(iG,jG))*sfac+addo
                !! lwdown_ESMF(ii,jj,1,1) = (ptr_lwdn(iG,jG))*sfac+addo
            end if
            if ((iG > 0 .and. iG < imax) .and.                          &
                (jG > 0 .and. jG < jmax) .and.                          &
                 ptr_swup(iG,jG) < TOL_R8 .and. ptr_swdn(iG,jG) < TOL_R8 ) then
                swdown_ESMF(ii,jj,1,1) = (ptr_swup(iG,jG)-ptr_swdn(iG,jG))*sfac+addo
            end if
            if ((iG > 0 .and. iG < imax) .and.                          &
                (jG > 0 .and. jG < jmax) .and. ptr_hl(iG,jG) < TOL_R8) then
                hl_ESMF(ii,jj,1,1) = -ptr_hl(iG,jG)*sfac+addo
            end if
            if ((iG > 0 .and. iG < imax) .and.                          &
                (jG > 0 .and. jG < jmax) .and. ptr_hs(iG,jG) < TOL_R8) then
                hs_ESMF(ii,jj,1,1) = -ptr_hs(iG,jG)*sfac+addo
            end if
            if ((iG > 0 .and. iG < imax) .and.                          &
                (jG > 0 .and. jG < jmax) .and. ptr_u10(iG,jG) < TOL_R8) then
                uwind_ESMF(ii,jj,1,1) = (ptr_u10(iG,jG)*sfac)+addo
            end if
            if ((iG > 0 .and. iG < imax) .and.                          &
                (jG > 0 .and. jG < jmax) .and. ptr_v10(iG,jG) < TOL_R8) then
                vwind_ESMF(ii,jj,1,1) = (ptr_v10(iG,jG)*sfac)+addo
            end if
            if ((iG > 0 .and. iG < imax) .and.                          &
                (jG > 0 .and. jG < jmax) .and. ptr_t2(iG,jG) < TOL_R8) then
                atemp_ESMF(ii,jj,1,1) = (ptr_t2(iG,jG)*sfac)+addo
            end if
            if ((iG > 0 .and. iG < imax) .and.                          &
                (jG > 0 .and. jG < jmax) .and. ptr_q2(iG,jG) < TOL_R8) then
                aqh_ESMF(ii,jj,1,1) = (ptr_q2(iG,jG)*sfac)+addo
            end if
            if ((iG > 0 .and. iG < imax) .and.                          &
                (jG > 0 .and. jG < jmax) .and. ptr_evap(iG,jG) < TOL_R8) then
                evap_ESMF(ii,jj,1,1) = ((- total_evap_ESMF(ii,jj,1,1)   &
                                          + ptr_evap(iG,jG))*sfac       &
                                            +addo) &
                                          / (esm_step_seconds+0.d0)     &
                                          / 1000.0d0
                total_evap_ESMF(ii,jj,1,1) = (ptr_evap(iG,jG))*sfac +addo
            end if
            if ((iG > 0 .and. iG < imax) .and.                          &
                (jG > 0 .and. jG < jmax) .and.                          &
                (ptr_rainc(iG,jG) < TOL_R8) .and.                       &
                (ptr_rainsh(iG,jG) < TOL_R8) .and.                      &
                (ptr_rainnc(iG,jG) < TOL_R8)) then
                precip_ESMF(ii,jj,1,1) = (- total_precip_ESMF(ii,jj,1,1)&
                                          + (ptr_rainc(iG,jG)           &
                                            +ptr_rainsh(iG,jG)          &
                                            +ptr_rainnc(iG,jG))*sfac    &
                                            +addo) &
                                          / (esm_step_seconds+0.d0)     &
                                          / 1000.0d0
                total_precip_ESMF(ii,jj,1,1) = (ptr_rainc(iG,jG)        &
                                               +ptr_rainsh(iG,jG)       &
                                               +ptr_rainnc(iG,jG))*sfac &
                                               +addo
            end if
            if ((iG > 0 .and. iG < imax) .and.                          &
                (jG > 0 .and. jG < jmax) .and. ptr_sst_ini(iG,jG) < TOL_R8) then
                sst_ini_ESMF(ii,jj,1,1) = (ptr_sst_ini(iG,jG)*sfac)+addo
            end if
          end do
        end do
!
!       ! Nullify the pointer
        ...
        if (associated(ptr_rainc)) then
          nullify(ptr_rainc)
        end if
        if (associated(ptr_rainsh)) then
          nullify(ptr_rainsh)
        end if
        if (associated(ptr_rainnc)) then
          nullify(ptr_rainnc)
        end if
        if (associated(ptr_sst_ini)) then
          nullify(ptr_sst_ini)
        end if
!
      end do       
...
!
      if (debugLevel >= 1) then
        ...
        call ESMF_FieldWrite(field_sst_ini, trim(ofile), rc=rc)
      end if

      DEALLOCATE(sst_ini_ESMF)
...
!
      ...
      ALLOCATE(sst_ini_ESMF(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy))

      call get_theta(theta_ESMF, myThid)
      call get_uvoce(uoce_ESMF, voce_ESMF, myThid)
      call get_sst_ini(sst_ini_ESMF, myThid)
!
!-----------------------------------------------------------------------
!     Get gridded component 
!-----------------------------------------------------------------------
        ...
        do jj = 1, sNy
          do ii = 1, sNx
            ...
            if (ptr_mask(iG,jG) == 1) then
              ...
              !! if land, use T2 in WRF
              !! ptr_sst(iG,jG) = ptr_t2(iG,jG)
              !! if land, use initial SST
              ptr_sst(iG,jG) = sst_ini_ESMF(ii,jj,1,1,1)
              ptr_uoce(iG,jG) = 0.d0
              ptr_voce(iG,jG) = 0.d0
            end if
          end do
