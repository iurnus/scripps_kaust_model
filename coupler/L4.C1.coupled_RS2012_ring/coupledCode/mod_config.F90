...
!
        ...
        call ESMF_ConfigGetAttribute(cf, wav_step_seconds,              &
                                     label='WAVStepSeconds:', rc=rc)
        call ESMF_TimeIntervalSet(wavTimeStep, s=wav_step_seconds, rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU,  &
            line=__LINE__, file=FILENAME)) return
!
        call ESMF_ConfigGetAttribute(cf, cpuWAV, label='cpuWAV:', rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU,  &
            line=__LINE__, file=FILENAME)) return

        call ESMF_ConfigGetAttribute(cf, waveNPX, label='waveNPX:', rc=rc)
        call ESMF_ConfigGetAttribute(cf, waveNPY, label='waveNPY:', rc=rc)
