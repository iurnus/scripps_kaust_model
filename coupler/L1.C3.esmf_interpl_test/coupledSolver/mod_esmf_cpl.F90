...
!
  ...
  if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU,    &
      line=__LINE__, file=FILENAME)) return
  PRINT *, "Field count is:, ", fieldCount
  ...
  do i=1, fieldCount
    call ESMF_FieldGet(fields(i), grid=grid, typekind=typekind, arrayspec=arrayspec, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU,  &
      line=__LINE__, file=__FILE__)) return  ! bail out
    ! field = ESMF_FieldCreate(grid, arrayspec, staggerloc=ESMF_STAGGERLOC_CENTER, rc=rc)
