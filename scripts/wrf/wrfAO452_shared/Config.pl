...
# put netcdfpar ahead of netcdf so that part of the name does not get clobbered
   ...
   if ( $sw_terrain_and_landuse )
    ...
    # RUI add a new flag WRF_ESMF = 1
    if ( $sw_esmflib_path && $sw_esmfinc_path || $ENV{WRF_ESMF} eq "1")
      {
...
while ( <ARCH_PREAMBLE> )
  ...
  # RUI add a new flag WRF_ESMF = 1
  elsif ( $ENV{WRF_ESMF} eq "1")
    {
    $_ =~ s/ESMFCOUPLING/2/g ;
    $_ =~ s:ESMFMODDEPENDENCE:\$\(WRF_SRC_ROOT_DIR\)/external/io_esmf/module_utility.o:g ;
    $_ =~ s:ESMFMODINC:-I\$\(ESMF_MOD\) -I\$\(WRF_SRC_ROOT_DIR\)/main -I\$\(WRF_SRC_ROOT_DIR\)/frame:g ;
    $_ =~ s:ESMFIOINC:-I\$\(WRF_SRC_ROOT_DIR\)/external/io_esmf:g ;
    $_ =~ s:ESMFIODEFS:-DESMFIO:g ;
    $_ =~ s:ESMFTARGET:wrfio_esmf:g ;
    $_ =~ s:\# ESMFINCLUDEGOESHERE:include \$\(ESMF_LIB\)/esmf.mk: ;

    $_ =~ s:CONFIGURE_ESMF_LIB_PATH:\$\(ESMF_LIB\)/libesmf.a : ;
    }
  else
    {
