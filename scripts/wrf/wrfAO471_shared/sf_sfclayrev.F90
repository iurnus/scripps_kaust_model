...
! AHW: change roughness length, and hence the drag coefficients Ck and Cd
       if(present(isftcflx)) then
          if(isftcflx.eq.1 .or. isftcflx.eq.2 .or. &
             isftcflx.eq.3 .or. isftcflx.eq.4) then
...
! AHW 2012:
          ...
          ! added by Rui for wave coupling                                             
          ! for MM5 sfc layer option                                                   
          elseif ( isftcflx.eq.7 ) then                                                
             waveLP0=MAX(waveLP(I),0.0083)                                            
             if (ocnMASK(I) .gt. 0.5) then                                            
               ZNT(I) = 1200.*waveHS(I)*(waveHS(I)/waveLP0)**4.5+0.11*1.5E-5/UST(I)   
             else                                                                     
               ZW  = MIN((UST(I)/1.06)**(0.3),1.0)                                    
               ZN1 = 0.011*UST(I)*UST(I)/G + OZO                                      
               ZN2 = 10.*exp(-9.5*UST(I)**(-.3333)) + &                               
             ...
             ZNT(I) = MAX( ZNT(I), 1.27e-7)  !These max/mins were suggested by        
             ZNT(I) = MIN( ZNT(I), 2.85e-3)  !Davis et al.  (2008)                    
          elseif ( isftcflx.eq.8 ) then                                                
             ZNT(I) = 2.85e-3                                                         
          elseif ( isftcflx.eq.9 ) then                                                
             ZNT(I) = 1.27e-7                                                         
          endif
       endif
       zl = znt(i)
