...
!                              PatchStart, PatchEndFull )
  ...
  SUBROUTINE ioesmf_create_grid_int( esmfgrid, numdims,      &
    ...
    REAL(ESMF_KIND_R8), DIMENSION(:,:), POINTER :: coordX2d_esmf, coordY2d_esmf
    type(ESMF_DistGrid) :: distGrid
    INTEGER, ALLOCATABLE :: deBlockList(:,:,:)
    INTEGER :: nI, nJ, iG, jG, tile

    PRINT *, "TESTBUG:: enter ioesmf_create_grid_int..."
...
! 5.2.0r      esmfgrid = ESMF_GridCreateShapeTile(  &
      ! PRINT *, 'TESTBUG:  i = ',i,'  dimXCount = ',dimXCount
      ! PRINT *, 'TESTBUG:  j = ',j,'  dimYCount = ',dimYCount
      ! PRINT *, 'TESTBUG:  i = ',i,'  dimXCount = ',dimXCount(0)
      ! PRINT *, 'TESTBUG:  j = ',j,'  dimYCount = ',dimYCount(0)
      ! PRINT *, 'TESTBUG:  allXStart = ', allXStart
      ! PRINT *, 'TESTBUG:  allYStart = ', allYStart
      ! PRINT *, 'TESTBUG:  allXCount = ', allXCount
      ! PRINT *, 'TESTBUG:  allYCount = ', allYCount
      ! PRINT *, 'TESTBUG:  globalXcount = ', globalXcount, ' globalYcount = ', globalYcount
      ...
      do tile = 0, (i*j)-1
        ...
        ! PRINT *, 'allYStart is: ', deBlockList(2,1,tile)
        ! PRINT *, 'allYEnd is: '  , deBlockList(2,2,tile)
      end do

      distGrid = ESMF_DistGridCreate(minIndex=(/ 1, 1 /), &
                                     maxIndex=(/ globalXcount, globalYcount /), &
                                     deBlockList=deBlockList, &
                                     rc=rc)

      esmfgrid = ESMF_GridCreate(distGrid=distGrid, &
      ...
      CALL ESMF_GridGetCoord(esmfgrid,coordDim=2,localDE=0, &
                 staggerloc=ESMF_STAGGERLOC_CENTER, &
                 !! computationalLBound=lbnd,computationalUBound=ubnd, &
                 farrayptr=coordY2d_esmf,                             &
                 rc=rc)
      !! DO i=lbnd(1),ubnd(1)
      !!   coordY2d(i) = (i-1)*1.0
      !! ENDDO

      do nJ = 1, allYCount(myPE)
        do nI = 1, allXCount(myPE)
          iG = allXStart(myPE) + nI - 1
          jG = allYStart(myPE) + nJ - 1 
          coordY2d_esmf(iG,jG) = head_grid%xlat(iG,jG)

          IF (head_grid%xlong(iG,jG) < 0) THEN
            coordX2d_esmf(iG,jG) = head_grid%xlong(iG,jG) + 360d0
          ELSE
            coordX2d_esmf(iG,jG) = head_grid%xlong(iG,jG)
          ENDIF

          ! PRINT *, "TESTBUG: i, j is : ", nI, nJ
          ! PRINT *, "TESTBUG: iG, jG is : ", iG, jG
          ! PRINT *, "TESTBUG: head_grid%xlat(iG,jG) is : ", head_grid%xlat(iG,jG)
          ! PRINT *, "TESTBUG: head_grid%xlong(iG,jG) is : ", head_grid%xlong(iG,jG)
        end do
      end do
...
!! PRINT *, 'TESTBUG: PRINT coordX and coordY'
!! DO i = 1, SIZE(coordX2d_esmf)
!!   PRINT *, 'DEBUG WRF:  coord1(',i,') = ', coordX2d_esmf(i)
!! ENDDO
!! DO j = 1, SIZE(coordY2d_esmf)
!!   PRINT *, 'DEBUG WRF:  coord2(',j,') = ', coordY2d_esmf(j)
!! ENDDO
...
!!       CALL wrf_debug ( 5 , TRIM(msg) )
  ...
  !! call ESMF_GridWriteVTK(esmfgrid, filename="atm_mesh_1",           &
  !!                        staggerLoc=ESMF_STAGGERLOC_CENTER, rc=rc)

  !! call ESMF_GridGetCoord(esmfgrid, staggerLoc=ESMF_STAGGERLOC_CENTER, &
  !!                        coordDim=1, array=arrX, rc=rc)
  !! call ESMF_GridGetCoord(esmfgrid, staggerLoc=ESMF_STAGGERLOC_CENTER, &
  !!                        coordDim=2, array=arrY, rc=rc)

  !! call ESMF_ArrayWrite(arrX, filename="atm_wrf_x1.nc",            &
  !!                      status=ESMF_FILESTATUS_NEW, rc=rc)
  !! call ESMF_ArrayWrite(arrY, filename="atm_wrf_y1.nc",            &
  !!                      status=ESMF_FILESTATUS_NEW, rc=rc)
