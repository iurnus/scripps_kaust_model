...
!  TYPE(ESMF_DataKind) :: esmf_kind
  ...
  ! REAL(ESMF_KIND_R4), POINTER :: tmp_sstini_r4_ptr_global(:,:)
  REAL(ESMF_KIND_R4), DIMENSION(:,:), ALLOCATABLE :: tmp_sstini_r4_ptr_global
  ...
  PRINT *, "TESTBUG: ext_esmf_write_field, location 1"
  ...
  PRINT *, "TESTBUG: ext_esmf_write_field, location 2"
...
!TODO:  implement this (below)
  ...
  PRINT *, "TESTBUG: ext_esmf_write_field, location 3"
...
! domain spec, so adjust if not staggered.
  ...
  IF ( .NOT. okay_to_write( DataHandle ) )  THEN
    ...
    PRINT *, "TESTBUG: ext_esmf_write_field, location case1 creating mesh"
    ...
    PRINT *, "TESTBUG: ext_esmf_write_field, location case1 exportstate"
    PRINT *, "TESTBUG: writing tmpField1... in case 1, ", TRIM(VarName)
    ...
    PRINT *, "TESTBUG: ext_esmf_write_field, creating tmpfield", trim(VarName)
    ...
    PRINT *, "TESTBUG: ext_esmf_write_field, location case1 add state"
    ...
    !! Added by Rui, save the initial SST field
    !! if (TRIM(VarName) == "SST") then
    !!     PRINT *, "case 1 iStep is: ", iStep
    !!     if (.not.allocated(data_esmf_real_ptr)) then
    !!       ALLOCATE( tmp_sstini_r4_ptr_global(ids:ide,jds:jde) )
    !!     end if
    !!     call ESMF_ArraySpecSet(sstiniarrayspec, typekind=ESMF_TYPEKIND_R4, &
    !!                            rank=2, rc=rc)

    !!     sstIniField = ESMF_FieldCreate(         &
    !!                   grid( DataHandle )%ptr, &
    !!                   sstiniarrayspec,              &
    !!                   staggerloc=ESMF_STAGGERLOC_CENTER,    &
    !!                   indexflag=ESMF_INDEX_GLOBAL, &
    ...
    !!     call ESMF_FieldGet(sstIniField, localDe=0, farrayPtr=tmp_sstini_r4_ptr_global, rc=rc)
    !!     !! tmp_sstini_r4_ptr_global = MISSING_R8
    !! end if

    IF ( rc /= ESMF_SUCCESS ) THEN
      CALL wrf_error_fatal("ext_esmf_write_field:  ESMF_StateAddfailed" )
    ENDIF
  PRINT *, "TESTBUG: ext_esmf_write_field, location case1 finished"
  ...
  ELSE IF ( okay_to_write( DataHandle ) )  THEN
    ...
    IF      ( FieldType .EQ. WRF_REAL ) THEN
      ...
      if ((TRIM(VarName) == "SST") .and. (iStep == 28)) then
          PRINT *, "case 2 iStep is: ", iStep
          if (.not.allocated(tmp_sstini_r4_ptr_global)) then
            ALLOCATE( tmp_sstini_r4_ptr_global(ids:ide,jds:jde) )
          end if
          !! tmp_sstini_r4_ptr_global = MISSING_R8
      end if
      ...
      do nJ = jps, jpe
        do nI = ips, ipe
          ...
          if ((TRIM(VarName) == "SST") .and. (iStep == 28)) then
            tmp_sstini_r4_ptr_global(nI,nJ) = data_esmf_real_ptr_global(nI,nJ)
          endif

          ! PRINT *, "TESTBUG: printing field name: ", trim(VarName)
          ...
          PRINT *, "TESTBUG: printing field name: ", trim(VarName)
          PRINT *, "TESTBUG: data_esmf_real_ptr_global is : ", data_esmf_real_ptr_global(1,1)
          PRINT *, "TESTBUG: data_esmf_real_ptr is : ", data_esmf_real_ptr(1,1)
      ...
      if ((TRIM(VarName) == "SST") .and. (iStep == 28)) then
          PRINT *, "TESTBUG: printing field name: ", trim(VarName)
          PRINT *, "TESTBUG: tmp_esmf_real_ptr_global is : ", tmp_sstini_r4_ptr_global(1,1)
          PRINT *, "TESTBUG: tmp_esmf_real_ptr is : ", tmp_sstini_r4_ptr_global(1,1)
      end if
